<template>
  <div class="card1">
    <div @click="changeMovie" class="mb-3 movieCard" @mouseover="showDetails = true" @mouseout="showDetails = false">
    <img :src="getImage" class="card-img-top" alt="NONONO" style="width: 100%; height: 100%;">
    <div class="card-overlay" :class="{ 'show-details': showDetails }">
      <h5 class="card-title">{{ genreMovie.title }}</h5>
      <h6>평점 {{ genreMovie.vote_average }}</h6>
      <p class="card-text">{{ truncateOverview }}</p>
    </div>
  </div>
  </div>
</template>

<script>
export default {
  name: 'GenreListItem',
  props: {
    genreMovie: Object
  },
  data() {
    return {
      showDetails: false
    }
  },
  computed: {
    getImage() {
      return `https://image.tmdb.org/t/p/original${this.genreMovie.poster_path}`
    },
    truncateOverview() {
      if (this.genreMovie.overview.length > 40) {
        return this.genreMovie.overview.slice(0, 40) + '...';
      } else {
        return this.genreMovie.overview;
      }
    }
  },
  methods: {
    changeMovie() {
      const movieinfo = this.genreMovie
      this.$store.dispatch('changeMovie', movieinfo)
      this.$router.push({ name: 'MovieDetail' })
    }
  }
}
</script>

<style scoped>
.movieCard {
  position: relative;
  display: block;
}

.card1 {
  display: block;
}

.card-overlay {
  position: absolute;
  top: 0;
  left: 0;
  width: 100%;
  height: 100%;
  background-color: rgba(0, 0, 0, 0.8);
  opacity: 0;
  transition: opacity 0.3s ease-in-out;
  display: flex;
  flex-direction: column;
  justify-content: center;
  align-items: center;
  text-align: center;

}

.show-details {
  opacity: 1;
}

.card-overlay h5,
.card-overlay h6,
.card-overlay p {
  margin: 0;
  padding: 0;
  color: white;
  margin-left: 10px;
  margin-right: 10px;
  margin-top: 5px;
}

.card-overlay h6{
  opacity: 0.8;
}

.card1 {
}
</style>